...
## Стандартный Python проект + poetry + isort + black + flake8 + autoflake

1. Собрать стандартный проект для `Python` и  `poetry + isort + black + flake8 + autoflake`:

    ```bash
    templaer -e_                              \
        -c  ./st/py_proj/пример_context.jsonc \
        -ti ./st/py_proj                      \
        -to ПапкаКудаСобрать                  
    ```
