...
from .pypars import TArgs, files_from_path, parse_args
...
def main(argv: list[str]):
    ...
    def _skip(_file):
        log("{c}Skip{r}:\t{f}".format(c=color.сyan.value, r=color.reset.value, f=_file))
    ...
    if path_in_template := cli_args.named_args.get("ti"):
        ...
        if path_out_template := cli_args.named_args.get("to"):
            ...
            for _files in files_from_path(path_in_template):
                ...
                write_name_file = pathlib.Path(_files.replace(str(path_in_template), str(path_out_template)))
                ...
                log("{y}Copy{r}:\t{f}".format(y=color.yellow.value, r=color.reset.value, f=write_name_file))
